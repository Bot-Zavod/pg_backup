...
function perform_backups()
{
    ...
    if [ "$RESTORE_DEV" = "yes" ]; then
        echo -e "\n\nDropping all tables in $DATABASE_DEV database"
        echo -e "--------------------------------------------\n"
        
        # drop all tables in DEV database
        # psql -U "$DB_USERNAME_DEV" -h "$DB_HOSTNAME" "$DATABASE_DEV" -t -c "select 'drop table \"' || tablename || '\" cascade;' from pg_tables where schemaname = 'public'" | psql -U "$DB_USERNAME" -h "$DB_HOSTNAME" "$DATABASE_DEV"
        if ! psql -U "$DB_USERNAME_DEV" -h "$DB_HOSTNAME" "$DATABASE_DEV" -t -c "select 'drop table \"' || tablename || '\" cascade;' from pg_tables where schemaname = 'public'" | psql -U "$DB_USERNAME" -h "$DB_HOSTNAME" "$DATABASE_DEV"; then
        ...
        if ! pg_restore --no-privileges --no-owner -h "$DB_HOSTNAME" -U "$DB_USERNAME" -d "$DATABASE_DEV" < "$FINAL_BACKUP_DIR""$DATABASE_PROD".custom; then
            ...
            if ! psql -U "$DB_USERNAME" -d "$DATABASE_DEV" -h "$DB_HOSTNAME" -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO $DB_USERNAME_DEV" ; then
                echo -e "[!!ERROR!!] Failed to ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO $DB_USERNAME_DEV" | log
            fi
            ...
            if ! psql -U "$DB_USERNAME" -d "$DATABASE_DEV" -h "$DB_HOSTNAME" -c "ALTER DEFAULT PRIVILEGES GRANT ALL ON TABLES TO $DB_USERNAME_DEV" ; then
                echo -e "[!!ERROR!!] Failed to ALTER DEFAULT PRIVILEGES GRANT ALL ON TABLES TO $DB_USERNAME_DEV" | log
            fi
        fi
    fi
